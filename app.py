...
def risco_continuo(row):
    if pd.isna(row["GeoSES"]) or pd.isna(row["Situacao_Calor"]):
        return None
    if row["Situacao_Calor"] == "Normal":
        peso_ehf = 0
    elif row["Situacao_Calor"] == "Calor Severo":
        peso_ehf = 0.5
    elif row["Situacao_Calor"] == "Calor Extremo":
        peso_ehf = 1
    else:
        return None
    return round(peso_ehf * (1 - row["GeoSES"]), 3)
...
df["Risco_Cont_EHF_GeoSES"] = df.apply(risco_continuo, axis=1)
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                options=[
                    ...
                    {"label": "Risco Combinado (Contínuo)", "value": "Risco_Cont_EHF_GeoSES"},
                ],
                value="Temp_Max",
                clearable=False
            )
...
def atualizar_mapa(variavel, data):
    ...
    fig = px.choropleth_mapbox(
        ...
        color_continuous_scale="OrRd" if variavel == "Risco_Cont_EHF_GeoSES" else ("RdBu_r" if "Min" in variavel else "Reds")
    )
